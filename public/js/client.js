...
const UI = {
    ...
    /**
     * Update survival result message (points awarded or not)
     * @param {boolean} awarded - Whether points were awarded
     * @param {number} points - Points value
     */
    updateSurvivalResult(awarded, points) {
        const el = document.getElementById('survival-team-status');
        if (!el) return;
        if (awarded) {
            el.textContent = `+${points} POINTS! Your team matched the majority!`;
...
const SocketHandlers = {
    ...
    init() {
        ...
        AppState.socket.on('survival_reveal', (data) => {
            ...
            if (data.is_tie) {
                UI.updateSurvivalStatus('TIE! No points awarded this round.');
                Haptics.success();
            } else if (teamAwarded) {
                UI.updateSurvivalResult(true, data.points_value);
                UI.updateSurvivalStatus(`Your team voted with the majority!`);
                Haptics.success();
            } else {
                UI.updateSurvivalResult(false, 0);
                const reason = teamNotAwarded?.reason;
                if (reason === 'team_tie') {
                    UI.updateSurvivalStatus('Your team was split - no majority!');
                } else if (reason === 'no_votes') {
                    UI.updateSurvivalStatus('No one from your team voted!');
                } else {
                    UI.updateSurvivalStatus('Your team voted with the minority.');
                }
                Haptics.impact();
            }
        });
